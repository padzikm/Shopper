...
const shop = (state = INITIAL_STATE, action) => {
    switch (action.type) {
        ...
        case CHECKOUT_SUCCESS:
            return Object.assign({}, state, { products: products(state.products, action), cart: cart(state.cart, action), error: '' });
        case CHECKOUT_FAILURE:
            return Object.assign({}, state, { error: 'Checkout failed. Please try again' });
        default:
            return state;
    }
};
...
const products = (state, action) => {
    switch (action.type) {
        ...
        case REMOVE_FROM_CART:
            ...
            return Object.assign({}, state, { [productId]: product(state[productId], action) });
        default:
            return state;
    }
};
...
const cart = (state, action) => {
    switch (action.type) {
        case ADD_TO_CART:
            ...
            return Object.assign({}, state, { [addId]: (state[addId] || 0) + 1 });
        case REMOVE_FROM_CART:
            ...
            if (quantity <= 0) {
                ...
                return Object.assign({}, state, { [removeId]: quantity });
            }
        case CHECKOUT_SUCCESS:
            return {};
        default:
